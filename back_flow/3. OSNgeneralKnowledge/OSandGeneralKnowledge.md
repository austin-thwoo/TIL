- [OS 와 일반적인 지식](#os-와-일반적인-지식)
  ...
  - [🔥\[5\] 쓰레드](#5-쓰레드)
    - [(1). 프로세스와 생성과 종료](#1-프로세스와-생성과-종료)
      - [{1-1}. 프로세스 생성](#1-1-프로세스-생성)
      - [{1-2}. 프로세스 종료](#1-2-프로세스-종료)
    - [(2). 쓰레드(Thread)](#2-쓰레드thread)
      - [{2-1}. 다중 쓰레드(Mulitithreads)](#2-1-다중-쓰레드mulitithreads)
      - [{2-2}. Thread VS Precess](#2-2-thread-vs-precess)
  - [🔥\[6\] 동시성과 병렬성 이해하기](#6-동시성과-병렬성-이해하기)
    - [(1) Parallelism](#1-parallelism)
    - [(2) Concurrency](#2-concurrency)
    - [(3) 동기화 문제는 Concurrent한 Task 사이에서 발생한다](#3-동기화-문제는-concurrent한-task-사이에서-발생한다)
    - [(4) 결론](#4-결론)
  - [🔥\[7\] 메모리 관리란? - 이유, 방법](#7-메모리-관리란---이유-방법)
      - [※ 메모리란?](#-메모리란)
      - [메모리 관리가 필요한 이유는?](#메모리-관리가-필요한-이유는)
        - [운영체제의 역할](#운영체제의-역할)
      - [Swapping](#swapping)
    - [(1)단편화 현상](#1단편화-현상)
      - [{1}. 내부 단편화](#1-내부-단편화)
      - [{2}. 외부 단편화](#2-외부-단편화)
    - [(2) 메모리 관리 방법](#2-메모리-관리-방법)
      - [{1}. 연속 메모리 할당](#1-연속-메모리-할당)
      - [{2}. 페이징(Paging)-가상메모리사용, 외부 단편화 해결, 내부 단편화 존재](#2-페이징paging-가상메모리사용-외부-단편화-해결-내부-단편화-존재)
      - [{3}. 세그먼테이션(Segmentation)-가상메모리사용, 내부 단편화 해결, 외부 단편화 존재](#3-세그먼테이션segmentation-가상메모리사용-내부-단편화-해결-외부-단편화-존재)
      - [{4}. 메모리 풀(Memory Pool)](#4-메모리-풀memory-pool)
  - [🔥\[8\] IPC란? - 정의, 종류, 방법](#8-ipc란---정의-종류-방법)
    - [(1). IPC의 종류](#1-ipc의-종류)
      - [{1}.메시지 전달(Message Passing)-메세지패싱](#1메시지-전달message-passing-메세지패싱)
      - [{2}. 메모리공유(Shared Memory)-쉐어드메모리](#2-메모리공유shared-memory-쉐어드메모리)
  - [🔥\[9\] OS - 입출력 관리](#9-os---입출력-관리)
    - [(1). 입출력 하드웨어의 구성](#1-입출력-하드웨어의-구성)
    - [(2).입출력 하드웨어의 동작](#2입출력-하드웨어의-동작)
      - [{1}. 플링](#1-플링)
      - [{2}. 인터럽트](#2-인터럽트)
      - [{3}. 입출력 하드웨어와 컴퓨터가 데이터를 주고받는 방식](#3-입출력-하드웨어와-컴퓨터가-데이터를-주고받는-방식)
...
IEEE는 전기 전자 기술자 협회라는데 이것은 넘어가고. POSIX를 풀어서 설명하자면 한 운영체제에서 개발한 프로그램을 다른 운영체제에서도 쉽게 돌아가도록 하는 표준이라고 한다.(소스코드의 호환)
